        ...
        return receiveBuilder()
                .match(ReadLines.class, r -> {
                    ...
                    for (int i = 0; i < lines.length; i++) {
                        String line = lines[i];
                        ActorRef wordCounterActorRef = getContext().actorOf(Props.create(WordCountActor.class), "word-counter-" + i);

                        CompletableFuture<Object> future =
                                ask(wordCounterActorRef, new WordCountActor.CountWords(line), 1000).toCompletableFuture();
                        futures.add(future);
                    }
